---
title: "Getting Started in R: Tinyverse (2nd Edition)"
...
view that _fewer is better_, and will rely on only two additional
(zero-dependency) packages: `data.table`[^datatable] for data wrangling as
well as input/output, and `tinyplot` [^tinyplot] for visualization.
...
To visualise the chick weight data, we will use the `tinyplot` package. Our interest is in seeing how
...
```{r emptyPlot, echo=2:8, fig.height=1.74}
op <- par(mfrow=c(1,2))
...
`Diet` (but not `Time`) a *factor variable*. This means that R will treat them as categorical
variables instead of continuous variables. It will simplify our coding. 

```{r cwfactor, echo=2:3, result="asis"}
options(digits=5,width=45)
cw[, Diet := factor(Diet)]  
...
## `facet` option
To plot each diet separately in a grid using `facet`:
...
`type_jitter(amount=1.0)`. Replace the (default) use of points by adding this
or its shorthand `type="jitter"`. What do you observe?
...
For the `legend` try using "top", "left" and "none". Do we really need a legend
for this plot?

## Mean line plot

Next we will plot the mean changes over time for each diet using the `type_summary()`
function (which defaults to using `mean`, and plotting lines):
...
What happens when you add `type = type_points()` to the plot above (via
`plt_add()`)? What about `type = type_jitter(amount = 1.0)`, possibly with explicit
jittering amounts or the default.

## Box-whisker plot

To see variation between the different diets we use `type_boxplot` to plot a box-whisker
...
* no legend (use `legend = "none"` (or `FALSE`)

## Final Plot 

Let us finish with a plot that one might include in a publication.
```{r finalPlot}
plt(weight ~ Time | Diet,
    data = cw,
    facet = "by",
...
[^tinyplot]: tinyplot: [https://grantmcdermott.com/tinyplot/](https://grantmcdermott.com/tinyplot/)
