...
This note is a variant of the original document[^gettingstarted] but stresses 
use of Base R along with careful dependency management as discussed below.
...
Alternatively, you can specify a full path, using `~` to denote your home directory.
...
Several base R functions help us inspect the data: `str()` compactly displays the
structure, `summary()` provides a summry and `head()` and tail()` display the beginning
and end of the data set.

```{r cwsummary, eval=FALSE, echo=FALSE}
str(cw)
summary(cw)
```

```{r cwsummaryActual, echo=2:3, eval=TRUE, result="asis"}
str <- function(object, ...) {
...
This shows that the dataset has `r nrow(cw)` observations and `r ncol(cw)` variables as we
would expect, and as compared to the original data file `ChickWeight.csv`.  So a good
start.
...
Let's make the summaries "prettier" for a possible report or publication.
...
following table. Its code uses the `kable` and `kableExtra` and is hidden here 
for compactness, full details are of course in the sources.
