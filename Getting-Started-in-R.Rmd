...
```{r cwfactor}
cw[, DietF := factor(Diet)]  # update by reference
cw[, Time := factor(Time)]
...
ggplot(cw, aes(Time, weight, colour=DietF)) +
  geom_point() +
  facet_wrap(~ Diet) +
  theme(legend.position = "bottom")
```
...
```{r dtarrange}
cw[, on=.(Weight)] # on the fly
setkey(cw, Chick, Time) # keyed
cw
```
