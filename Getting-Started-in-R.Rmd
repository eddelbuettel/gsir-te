...
```{r writeCW, echo=FALSE, message=FALSE, warning=FALSE}
ChickWeight %>%
...
```{r dtrename}
setnames(cw2, c("Diet", "weight"), c("Group", "Weight"))
cw2[]
```
...
```{r cwchained}
cw3 <- copy(cw)
cw3 <- cw3[ Time %between% c(0,21)]
cw3[, Weight := weight]
cw3[, Group := factor(paste("Diet", Diet))]
cw3[, .(Chick, Group, Time, Weight)][,on=.(Chick,Time)]
```
...
```{r ctstats}
cw4 <- copy(cw)
cwSum <- cw4[ , .(N = .N, Mean = mean(weight)), by=.(Diet, Time)]
head(cwSum)
```
...
```{r dtpretty}
cwSum[, Mean_SD := paste0(format(Mean, digits=1), " (", format(SDev, digits=2), ")")]
cwSum[, Range := paste(Min, "-", Max)]
prettyCWSum <- cwSum[, .(Diet, Time, N, Mean_SD, Median, Range), on=.(Diet, Time)]
head(prettyCWSum, 10)
```
...
tbl <- kable(prettyCWSum[Time %in% c(0,21), ], "latex", booktabs = TRUE, linesep = "", align = "cccrc") 
