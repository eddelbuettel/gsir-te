...
and documentation. We will use the `tidyverse` package[^tidyverse] to see this in practice.
...
You must have the "`tidyverse`" package installed to continue. Please install it now
using:
...
To be able to use the `tidyverse` (or any other) package you have use the `library()` 
function to load it. Please run the following in R:
```{r usePackage}
library(tidyverse)
```
